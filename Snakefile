...
if config["mode"] == "full":
    rule wssd_merge:
        ...
        run:
            ...
            shell("python3 scripts/merger.py {tempfile} --infiles {input.wssd} --wssd_merge --contigs_file {CONTIGS_FILE}")
    ...
    rule merge_sparse_matrices:
        ...
        run:
            ...
            shell('python3 scripts/merger.py {tempfile} --infile_glob "{infile_glob}" --contig {wildcards.contig}')
            shell("rsync {tempfile} {output}")
    ...
    rule map_and_count:
        ...
        run:
            ...
            shell("{SNAKEMAKE_DIR}/bin/bam_chunker -b {input.bam} -i {local_index} -p {wildcards.part} -n {BAM_PARTITIONS} -u {UNMAPPED_PARTITIONS} 2>> /dev/stderr | "
                ...
                "python3 scripts/read_counter.py {fifo} {output} {CONTIGS_FILE} {read_counter_args}"
                )
